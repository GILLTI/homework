...
public class Mon2 {
	    ...
	    public static void main(String[] args) {
	        ...
	        String result = mt.signUp(userIdArray, userNameArray, userId, userName, password, confirmPassword);
	        System.out.println(result); //최종 결과 표시
	    }

	    //회원가입을 진행하기 위한 메서드, 각 매개변수에서는 중복확인하기 위한 배열2개와 입력한 값이 저장된 변수를 지정
	    //어차피 signup메서드 내부에서 사용하는것이기 때문에 똑같이 해도 상관 없다
	    String signUp(String[] userIdArray, String[] userNameArray, String userId, String userName, String password, String confirmPassword) {
	        ...
	        }else if (!password.equals(confirmPassword)) { // 비밀번호 검사이다. else if문으로 해야 비번 검사 조건식이 들어가진다
	    ...
	    String convertToAscii(String input) { //input을 매개변수
	        ...
	        for (int i=0; i<input.length(); i++) { //비밀번호를 받아오면 for문을 통해 String을 char로 변환 후 아스키로 변환해주기
	        	char ch = input.charAt(i); //비번의 길이만큼 변환 → String에서 char로
